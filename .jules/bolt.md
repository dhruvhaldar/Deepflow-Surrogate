...
## 2024-10-25 - In-Place Evaluation Overcomes Monolithic Overhead for Complex Equations
**Learning:** We previously found that monolithic expression evaluation outperformed in-place Horner's method because we were assigning a contiguous array expression to a non-contiguous slice. However, monolithic evaluation of a 4th-order polynomial (`np.sqrt(x)*c0 + x*(c1 + x*...)`) implicitly generates 6+ large intermediate temporary arrays in memory. Replacing the monolithic evaluation with explicit in-place ufuncs (`np.multiply(x, c4, out=out)`, `np.add(...)`) completely avoids these heavy memory allocations, and even for non-contiguous column slices of Fortran arrays, bypassing these allocations results in a ~2x speedup over the monolithic approach for very large datasets.
**Action:** When a pre-allocated array slice (`out`) is provided to a complex NumPy polynomial or mathematical function, prioritize using explicit, sequential in-place ufuncs (like `np.multiply(..., out=out)`) instead of evaluating the entire expression monolithically. This trades native C-level expression evaluation for a massive reduction in memory allocation overhead, yielding significant performance gains.

## 2024-05-19 - NumPy Ufunc Temporary Allocations in Expressions
**Learning:** Even when using explicit `out` parameters for sequential `numpy` ufuncs, evaluating a single sub-expression like `np.sqrt(x) * c0` dynamically inside an `np.add(out, ...)` call implicitly forces `numpy` to allocate a full-sized temporary array for the intermediate result before the addition.
**Action:** Always pre-allocate a `scratch` buffer (or reuse an existing, no-longer-needed array like the initial linear space `x`) and pass it down to performance-critical mathematical functions. Use explicit sequenced ufuncs (e.g., `np.sqrt(x, out=scratch)`, `np.multiply(scratch, c0, out=scratch)`) to ensure strict zero-allocation execution paths, saving significant memory bandwidth and execution time on large arrays.

## 2026-03-16 - Numpy Augmented Assignment for In-Place Operations
**Learning:** We previously optimized complex NumPy math by using explicit ufuncs with an `out=` buffer (e.g. `np.multiply(x, c4, out=out)`) to avoid allocating heavy temporary arrays in memory. However, calling `np.multiply` explicitly carries Python function-call overhead. Using Python's augmented assignment operators (`*=`, `+=`) directly on the target NumPy array natively maps to NumPy's highly-optimized in-place C-level evaluation but sidesteps the function-call overhead, yielding a ~15-20% speedup for dense polynomial evaluation blocks without sacrificing the zero-allocation memory properties.
**Action:** When performing dense sequential math operations on a pre-allocated array (e.g., polynomial evaluation via Horner's method), prefer Python augmented assignment operators (`out += c3`, `out *= x`) over explicitly calling `np.add` and `np.multiply` to bypass the function call overhead.
...
## 2026-04-17 - Map Outperforms List Comprehension for C API Calls
**Learning:** When repeatedly calling a C API function (like `gmsh.model.geo.addPoint`) thousands of times with multiple arguments from parallel lists in Python, using `list(map(add_point, xs, ys, [0.0]*len(xs), [lc]*len(xs)))` is measurably faster than a list comprehension `[add_point(x, y, 0.0, lc) for x, y in zip(xs, ys)]`. This is because `map` pushes the iteration entirely down to C level, avoiding the overhead of evaluating Python bytecode for the loop and `zip` generator on every single iteration.
**Action:** When a bottleneck involves mapping parallel list data to a simple function or C extension call without internal Python logic, prefer `map()` over list comprehensions or explicit `for` loops.
