...
## 2024-05-18 - C API Repeated Calls Optimization
**Learning:** When repeatedly calling a C API function (like `gmsh.model.geo.addPoint`) thousands of times with multiple arguments from parallel lists in Python, using `list(map(func, list1, list2, ...))` with pre-allocated lists for constant arguments (e.g., `[0.0] * len(xs)`) is significantly faster (~1.9x) than using a list comprehension with `zip()`. `map()` avoids Python's loop bytecode evaluation overhead by pushing the iteration entirely to the C level.
**Action:** When working with large datasets and C API interactions in Python, replace list comprehensions containing `zip()` with `map()` paired with pre-allocated lists for constant arguments.
