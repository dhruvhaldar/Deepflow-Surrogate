...
## 2025-05-28 - Zero-Allocation Path using Scratch Buffers in Vectors
**Learning:** Even when using explicit `out` parameters for sequential `numpy` ufuncs, evaluating a sub-expression like `np.sqrt(x) * c0` implicitly forces `numpy` to allocate a full-sized temporary array.
**Action:** To achieve strict zero-allocation paths, pass a `scratch` buffer (reusing an existing, no-longer-needed array like an initial linear space `x` is much faster than allocating a new one) and evaluate sub-expressions with sequenced ufuncs (e.g., `np.sqrt(x, out=scratch)`).
