...
## 2026-03-22 - Vectorized Math Array Allocation Trade-off
**Learning:** Monolithic NumPy expressions evaluate efficiently but allocate large temporary arrays for the final result. While assigning to non-contiguous array slices using `out[:]` is fast, splitting the monolithic expression into sequential in-place steps (like `np.sqrt(x, out=out)`, `out *= c0`, `out += ...`) combined with iterative polynomial evaluation avoids creating a full-size intermediate array before assignment. This hybrid approach yielded a ~22% speed boost over the monolithic `out[:]` assignment.
**Action:** When calculating complex polynomials into pre-allocated memory, use sequential in-place math and evaluate the polynomial using intermediate temporary arrays instead of a single monolithic expression to avoid peak memory allocations and improve CPU cache locality.
