...
class TestInteractiveSave(unittest.TestCase):
    ...
    @patch('sys.stdout.isatty', return_value=True)
    @patch('mesh_generation.ensure_directory_exists')
    def test_interactive_save_custom_quoted(
        self, mock_ensure, mock_isatty, mock_in, mock_chk, mock_sz
    ):
        """Test that user can drag-and-drop a quoted path interactively."""
        # pylint: disable=unused-argument
        ...
        with patch.dict('sys.modules', {'gmsh': mock_gmsh}):
            ...
            # Verify the quotes were stripped before validation and passing to write
            mock_chk.assert_called_with("custom_mesh.msh", force=False)
            mock_ensure.assert_called_with("custom_mesh.msh")
...
class TestArgparseError(unittest.TestCase):
    ...
    def test_argparse_error_output(self, mock_stderr):
        ...
        try:
            mesh_generation.main(['--unknown-flag'])
        except SystemExit:
            pass
